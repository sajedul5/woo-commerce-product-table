 ...
 *   https://datatables.net/download/#dt/dt-1.10.18/af-2.3.3/r-2.2.2
 *
 * Included libraries:
...
(function(h){"function"===typeof define&&define.amd?define(["jquery"],function(E){return h(E,window,document)}):"object"===typeof exports?module.exports=function(E,H){E||(E=window);H||(H="undefined"!==typeof window?require("jquery"):require("jquery")(E));return h(H,E,E.document)}:h(jQuery,window,document)})(function(h,E,H,k){function Z(a){var b,c,d={};h.each(a,function(e){if((b=e.match(/^([^A-Z]+?)([A-Z])/))&&-1!=="a aa ai ao as b fn i m o s ".indexOf(b[1]+" "))c=e.replace(b[0],b[2].toLowerCase()),
    ...
    Z(n.defaults.column);n.models.oSettings={oFeatures:{bAutoWidth:null,bDeferRender:null,bFilter:null,bInfo:null,bLengthChange:null,bPaginate:null,bProcessing:null,bServerSide:null,bSort:null,bSortMulti:null,bSortClasses:null,bStateSave:null},oScroll:{bCollapse:null,iBarWidth:0,sX:null,sXInner:null,sY:null},oLanguage:{fnInfoCallback:null},oBrowser:{bScrollOversize:!1,bScrollbarLeft:!1,bBounding:!1,barWidth:0},ajax:null,aanFeatures:[],aoData:[],aiDisplay:[],aiDisplayMaster:[],aIds:{},aoColumns:[],aoHeader:[],
        ...
        classes:{},build:"dt/dt-1.10.18/af-2.3.3/r-2.2.2",errMode:"alert",feature:[],search:[],selector:{cell:[],column:[],row:[]},internal:{},legacy:{ajax:null},pager:{},renderer:{pageButton:{},header:{}},order:{},type:{detect:[],search:{},order:{}},_unique:0,fnVersionCheck:n.fnVersionCheck,iApiIndex:0,oJUIClasses:{},sVersion:n.version};h.extend(x,{afnFiltering:x.search,aTypes:x.type.detect,ofnSearch:x.type.search,oSort:x.type.order,afnSortData:x.order,aoFeatures:x.feature,oApi:x.internal,oStdClasses:x.classes,oPagination:x.pager});
...
(function ($) {
    ...
    function pfw_category_filter_on_click()
    {

        // e.preventDefault();
        //check if the ul was inserted
        if ($(this).find('.pfw-select-checkbox-options').length === 0)
        {
            let select_div = $(this);
            let select_filter = $(this).find('select');

            console.log(select_filter.html());
            let selected = select_filter.val() || [];
            let options = select_filter.find('option');
            ...
            _.each(options, function(_option){
                ...
                var option_selected = selected.indexOf(option.val()) !== -1 ? 'checked' : '';
                ul_html += '<li><input type="checkbox" '+option_selected+' value="'+option.val()+'" /> '+option.html()+' </li>';
            });

            ul_html = '<div class="pfw-select-checkbox-options"><ul>'+ul_html+'</ul> <span class="pfw-category-filter-close-button">Close</span></div>';
            select_div.append(ul_html);
        }
    ...
    $(function(){
        ...
        //redraw the table on filters
        $(document).on('change','.pfw-filters select', function(){
           let tableID = '#' + $(this).closest('.pfw-product-table > div').attr('id');
            console.log(tableID, 'is the table id');
           if (typeof pfw_tables[tableID] !== 'undefined')
               pfw_tables[tableID].draw();
        });

        $(document).on('keyup','.pfw-filters input[type=text]', function(){
        ...
        $(document).on('click', '.pfw-filter-categories', pfw_category_filter_on_click);

        $(document).on('click', '.pfw-category-filter-close-button', function (e) {
            $(this).closest('.pfw-select-checkbox-options').remove();
            e.stopPropagation();
        });

        $(document).on('change', '.pfw-select-checkbox-options input[type=checkbox]', function(e){
            //now select the options accordingly
            let closest_select = $(this).closest('.pfw-filter-categories').find('select').first();
            let options = closest_select.find('option');

            console.log('options length: ', options.length);
            let checked = $(this).is(':checked');
            let checkbox_val = $(this).val();
            console.log('checked is: ', checked);
            console.log('checked value is: ', checkbox_val);
            _.each(options, function(_option){
                let option = $(_option);
                console.log('option value is: ', option.val());
                console.log('they are equal, ', option.val() === checkbox_val);
                if (option.val() === checkbox_val)
                {
                    if (checked)
            ...
            closest_select.trigger('change');

        });

        //On clear button clicked, reset all disabled options
        $(document).on('click', '.pfw-clear-variation-select', function(){
            let parent_div = $(this).closest('.pfw-add-to-cart-group').find('.pfw-variations');
            parent_div.find('select').prop('selectedIndex',0);
            parent_div.find('select option').prop('disabled', false);
        });

        $(document).on('change', '.pfw-product-table td .pfw-variations select', function(){

            let this_select_name = $(this).attr('name');
            //filter only available variations
            let parent_div = $(this).closest(".pfw-variations");
            let available_variations = JSON.parse(parent_div.attr('data-variations'));

            //remove all disabled from the selects
            parent_div.find('select option').prop('disabled', false);


            console.log('available variations: ', available_variations);
            let all_selects = parent_div.find('select');

            //get all current value of all available selects. This will be used to filtered later
            let selected_values = {};
            _.each(all_selects, function(select){
                if ($(select).val() !== '')
                    selected_values[$(select).attr('name')] = $(select).val();
            });

            console.log(selected_values);
            //now we get the current selected values, run through all available variation to disable not available options
            let valid_variations = available_variations.filter(function(variation){
               let keys = Object.keys(selected_values);
               let match = true;

               for (let i = 0; i < keys.length; i++)
               {
                   if (variation.attributes[keys[i]] !== selected_values[keys[i]])
                   {
                       match = false;
                       break;
                   }
            ...
            let valid_array = [];
            //filter other selects, not this one
            _.each(valid_variations, function(valid){
                delete valid.attributes[this_select_name];

                valid_array.push(valid.attributes);
            });

            console.log('valid array', valid_array);


            let other_selects = parent_div.find('select[name!="'+this_select_name+'"]');


            _.each(other_selects, function (select) {
                ...
                _.each(options, function(option){
                    ...
                    {
                        let available = false;
                        for (let i = 0; i < valid_array.length; i++)
                        {
                            if (valid_array[i][select_name] === option_value)
                            {
                                available = true;
                                break;
                            }
            ...
            //get the table and current row
            let tableID = '#' + $(this).closest('.pfw-product-table > div').attr('id')
            let table = pfw_tables[tableID];
            ...
            //first, get all the select boxes and check if there value are all set
            var selects = $(this).closest('.pfw-variations').find('select');

            var not_all_value_are_set = false;
            var attributes = {};
            _.each(selects, function(select){
                if (typeof $(select).val() === 'undefined' || $(select).val().trim() === '')
                {
                    not_all_value_are_set = true;
                }

                attributes[$(select).attr('name')] = $(select).val();
            });

            if (not_all_value_are_set)
            {
                console.log('not all values are set');
                return;
            }


            var data_div = $(this).closest('.pfw-add-to-cart-group');

            var parent_product_id = data_div.attr('data-parent-product-id');
            var price_field = data_div.closest('tr').find('.pfw-product-price').first();

            data_div.addClass('waiting-response');
            if (typeof price_field !== 'undefined')
                price_field.addClass('waiting-response');

            $.post(ajaxurl, {
                'action' : 'pfw_get_product_variation_id',
                'attributes': attributes,
                'product_id': parent_product_id


            }, function(response){
                var selected_product = JSON.parse(response);
                ...
                if (typeof price_field !== 'undefined')
                {
                    price_field.html(selected_product.price_html);
                    price_field.removeClass('waiting-response');
                }

                //update the table data


                console.log('row data is: ', row_data);
                row_data.raw_price = selected_product.display_price;
                row_data.price = {
                    'display' : selected_product.price_html,
                    'value': selected_product.display_price
                };

                //update product id
                data_div.attr('data-product-id', selected_product.variation_id);
                data_div.removeClass('waiting-response');
            })

            //if the value are set, get the product details and update the price


            // price_box.html(selected_variation.price_html);
        ...
        //only apply on button, not on link. For grouped and affiliate product, the button is a <a tag
        $(document).on('click', '.pfw-product-table button.add_to_cart_button', function(e){

            e.preventDefault();


            var selects = $(this).closest('.pfw-add-to-cart-group').find('.pfw-variations select');
            ...
            if (not_all_value_are_set)
            {
                iziToast.error({
                    title: 'Error',
                    message: 'Please set all the variations'
                });
                return;
            }
            ...
            var product_type = data_div.attr('data-type');
            var product_id = data_div.attr('data-parent-product-id');
            if (product_type === 'variable')
                product_id = data_div.attr('data-product-id');

            var quantity = data_div.find('.pfw-order-quantity').val();
            var order_data = {
            ...
            $.post(window.location.href + "?wc-ajax=add_to_cart" , order_data, function(response){
        ...
        /*******************************
         LIGHT BOX FOR PRODUCT IMAGE
         *******************************/

        let lightcaseInit = false;
        $(document).on('mouseover', '.pfw-lightcase', function(){
            if (!lightcaseInit)
            {
                $('a.pfw-lightcase').lightcase();
                lightcaseInit = true;
            }
        });
        // $('.pfw-product-image').lightcase();
        $(document).on('click', '.pfw-product-image', function(e){
            e.preventDefault();
        });

        //bulk add button
        $(document).on('click', '.pfw-bulk-add-button', function(){

            let button = $(this);
            //check if every thing products variation are set
            let table = $(this).closest('.pfw-product-table');

            let selected_product_add_to_cart_div = [];

            _.each(table.find('.pfw-product-checkbox[type=checkbox]:checked'), function(box){
                selected_product_add_to_cart_div.push($(box).closest('tr').find('.pfw-add-to-cart-group'));
            });

            //create an array to store the variable products
            let invalid_variable_product_rows = [];
            //check the data div and see if there are variable products and they have variation set
            _.each(selected_product_add_to_cart_div, function(data_div){
                data_div = $(data_div);
                let product_type = data_div.attr('data-type');
                if (product_type === 'variable' && typeof data_div.attr('data-product-id') === 'undefined' || parseInt(data_div.attr('data-product-id')) === 0)
                {
                    invalid_variable_product_rows.push(data_div.closest('tr'));
                }

            });

            if (invalid_variable_product_rows.length > 0)
            {
                console.log('ooops');
                //if there are error rows, add the error class and return
                _.each(invalid_variable_product_rows, function(row){
                    $(row).addClass('pfw-error-row');

                    setTimeout(function(){
                        $(row).removeClass('pfw-error-row');
                    }, 4000);
            ...
            //now add all products to cart
            let products = [];

            _.each(selected_product_add_to_cart_div, function(data_div){
                let qty = $(data_div).find('.pfw-order-quantity').first().val();

                let product_id = $(data_div).attr('data-parent-product-id');

                if (data_div.attr('data-type') === 'variable')
                    product_id = $(data_div).attr('data-product-id');
            ...
            jQuery.post(ajaxurl, {
                action: 'pfw_add_items_from_cart',
                products: products
            }, function(response){
